...
public class UserAndLoginController {
    ...
    public String modifyitem(
        ...
        LOG.debug("get viewModifyItem called for username=" + username);
        ...
        if (UserRole.ANONYMOUS.equals(sessionUser.getUserRole())) {
            errorMessage = "you must be logged in to access item information";
            model.addAttribute("errorMessage", errorMessage);
            return "home";
        }
        ...
        if (!UserRole.ADMINISTRATOR.equals(sessionUser.getUserRole())) {
            // if not an administrator you can only access your own account info
            if (!sessionUser.getUsername().equals(username)) {
                errorMessage = "security non admin viewModifyItem called for username " + username
                        + "which is not the logged in user =" + sessionUser.getUsername();
                LOG.warn(errorMessage);
                model.addAttribute("errorMessage", errorMessage);
                return ("home");
        ...
        List<User> userList = userRepository.findByUsername(username);
        if (userList.isEmpty()) {
            LOG.error("viewModifyItem called for unknown username=" + username);
            return ("home");
        }
    ...
    public String updateitem(
        ...
        LOG.debug("post updateItem called for username=" + username);
        ...
        if (!UserRole.ADMINISTRATOR.equals(sessionUser.getUserRole())) {
            if (!sessionUser.getUsername().equals(username)) {
                errorMessage = "security viewModifyItem called for non admin username " + username
                        + "which is not the logged in user =" + sessionUser.getUsername();
                model.addAttribute("errorMessage", errorMessage);
                LOG.warn(errorMessage);
                return ("home");
        ...
        List<User> userList = userRepository.findByUsername(username);
        if (userList.isEmpty()) {
            errorMessage = "update item called for unknown username:" + username;
            LOG.warn(errorMessage);
            model.addAttribute("errorMessage", errorMessage);
            return ("home");
        }
